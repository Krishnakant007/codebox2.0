...
function CourseStatus({ courseDetail }: Props) {
    ...
    const GetCounts = () => {
        ...
        courseDetail?.chapters?.forEach((chapter) => {
            ...
            // Count exercises
            totalExcercises += chapter?.exercises?.length || 0

            // Count XP
            chapter?.exercises?.forEach((exc: any) => {
                totalXp += exc?.xp || 0
            })
        })
        ...
        <div className='font-game p-4 border-4 rounded-xl w-full'>
            ...
            <div className='flex items-center gap-5 mt-4'>
                ...
                <div className='w-full'>
                    <h2 className='flex justify-between text-2xl'>
                        XP Earned
                        <span className='text-gray-400'>{courseDetail?.courseEnrolledInfo?.xpEarned}/{counts.totalXp}</span>
                    </h2>
                    {/* //@ts-ignore */}
